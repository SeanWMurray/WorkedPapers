...
pub async fn sign_off(
    ...
    db.transaction(|conn| {
        conn.execute(
            "INSERT INTO signoffs (scope, role, signed_by, signed_at, signature_hash)
             VALUES (?1, ?2, ?3, ?4, ?5)",
            params![scope, role.to_uppercase(), signed_by, now, hash],
        )?;
...
pub async fn get_signoffs(
    ...
    let signoffs: Vec<Signoff> = if let Some(s) = scope {
        let mut stmt = db.conn.prepare(
            "SELECT id, scope, role, signed_by, signed_at, signature_hash
